...
def getMaillingList(ip, reqCookie):
    ...
    try:
        r = requests.get('http://' + ip + ':8080' + '/rest/v1/mailing-lists', headers=myHeader, cookies=reqCookie)
...
def getMaillingListByXid(ip, reqCookie, Xid):
    ...
    myHeader = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                'X-XSRF-TOKEN': reqCookie['XSRF-TOKEN']}
    try:
        r = requests.get('http://' + ip + ':8080' + '/rest/v1/mailing-lists/' + Xid, headers=myHeader, cookies=reqCookie)
        if r.status_code == 200 or r.status_code == 201:
            return r.content
        else:
            print "error occurred"
